(function () {
  ...
  function RelationshipController($ionicHistory, $ionicModal, $ionicPopup, $location, $log, $scope, $state,
    ...
    function activate() {
      ...
      $ionicModal.fromTemplateUrl('app/project/select-relationship-types-modal.html', {
        'scope': $scope,
        'animation': 'slide-in-up'
      }).then(function (modal) {
    ...
    function go(path) {
      if (Object.keys(vm.data).length > 1) {
        ...
        else if (!vm.data.a || !vm.data.b || !vm.data.types) {
          $ionicPopup.alert({
    ...
    function isOptionChecked(item, id, parentSpotId) {
      ...
      if (item === 'features') {
        if (vm.data[order][item] && vm.data[order][item][parentSpotId]) return _.contains(
