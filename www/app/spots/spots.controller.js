  ...
  function Spots($cordovaDevice, $cordovaFile, $document, $ionicHistory, $ionicLoading, $ionicModal, $ionicPopover,
    ...
    function convertToCSV() {
      ...
      _.each(vm.spots, function (spot) {
        ...
        else allHeaders = _.union(headers, allHeaders);
      });
        ...
        _.each(spot.properties, function (value, key) {
          ...
          if (_.isObject(value)) {
            ...
            if (value instanceof Date) {
              if (key === 'time') row[i] = value.toLocaleTimeString();
              else if (key === 'date') row[i] = value.toLocaleDateString();
              else row[i] = value.toJSON();
            }
            // Flatten the child values
            else row[i] = '\'' + _.flatten(value).join(', ') + '\'';
          }
          else row[i] = '\'' + value + '\'';
        });
